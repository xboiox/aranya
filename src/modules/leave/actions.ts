...
async function decide(
  ...
  return decideApproval({
    ...
    rejectionReason,
    entityType: "leave_request",
    auditAction: { approved: "leave.approve", rejected: "leave.reject" },
    successMessage: { approved: "Pengajuan disetujui", rejected: "Pengajuan ditolak" },
    revalidate: ["/dashboard/leave/approvals", "/dashboard/leave"],
    load: (tx, recordId) =>
      tx.query.leaveRequests.findFirst({ where: eq(leaveRequests.id, recordId) }),
    update: async (tx, recordId, patch) => {
      await tx.update(leaveRequests).set(patch).where(eq(leaveRequests.id, recordId))
    },
    notification: (req, dec, reason) => ({
      type: `leave_${dec}`,
      title: dec === "approved" ? "Cuti disetujui" : "Cuti ditolak",
      body:
        dec === "approved"
          ? `Pengajuan ${leaveTypeLabel(req.type)} ${req.totalDays} hari Anda disetujui.`
          : `Pengajuan ${leaveTypeLabel(req.type)} Anda ditolak.${reason ? ` Alasan: ${reason}` : ""}`,
