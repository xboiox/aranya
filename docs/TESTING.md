...
| **RLS isolasi tabel Modul 2**: `assets`, `onboarding_tasks`, dan tabel KPI berjenjang (`kpi_scorecards`, `kpi_tasks`, `kpi_progress`, `kpi_feedback`, `kpi_appraisals`) — baca terisolasi, tanpa context kosong, bypass lihat semua, tulis lintas-tenant ditolak | `src/test/integration/module2-rls.integration.test.ts` |
